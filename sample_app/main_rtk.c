...
#define PNET_MAIN_LOG            LOG_STATE_ON
...
{
   LOG_INFO(PNET_MAIN_LOG, "APP: connect call-back %d %d %d %d\n",
...
static int app_release_ind(
   ...
   pnet_result_t           *p_result)
{
   LOG_INFO(PNET_MAIN_LOG, "APP: release call-back %d %d %d %d\n",
         p_result->pnio_status.error_code,p_result->pnio_status.error_decode,
         p_result->pnio_status.error_code_1,p_result->pnio_status.error_code_2);
...
static int app_dcontrol_ind(
   ...
   pnet_result_t           *p_result)
{
   LOG_INFO(PNET_MAIN_LOG, "APP: dcontrol call-back (cmd:%d) %d %d %d %d\n",
         control_command,
         p_result->pnio_status.error_code,p_result->pnio_status.error_decode,
         p_result->pnio_status.error_code_1,p_result->pnio_status.error_code_2);
...
static int app_ccontrol_cnf(
   ...
   pnet_result_t           *p_result)
{
   LOG_INFO(PNET_MAIN_LOG, "APP: ccontrol confirmation call-back %d %d %d %d\n",
         p_result->pnio_status.error_code,p_result->pnio_status.error_decode,
         p_result->pnio_status.error_code_1,p_result->pnio_status.error_code_2);
...
static int app_write_ind(
   ...
   pnet_result_t           *p_result)
{
   LOG_INFO(PNET_MAIN_LOG, "APP: write call-back: idx %u\n", (unsigned)idx);
   if (idx == APP_PARAM_IDX_1)
   {
      ...
      {
          LOG_INFO(PNET_MAIN_LOG, "APP: write call-back: idx %u  length %u - expected length %u\n",
   ...
   {
      LOG_INFO(PNET_MAIN_LOG, "APP: write call-back: unknown idx %u\n", (unsigned)idx);
   }
   return 0;
}
...
static int app_read_ind(
   ...
   pnet_result_t           *p_result)
{
   LOG_INFO(PNET_MAIN_LOG, "APP: read call-back: idx %u  length %u\n",
   ...
   pnet_event_values_t     state)
{
   ...
   LOG_DEBUG(PNET_MAIN_LOG, "APP: state call-back %d\n", state);
   ...
   uint32_t                module_ident)
{
   ...
   if (ix < NELEMENTS(cfg_module_ident_numbers))
   {
      ...
      {
         LOG_DEBUG(PNET_MAIN_LOG, "APP: Pull module failed\n");
      }
      /* For now support any module in any slot */
      ret = pnet_plug_module(api, slot, module_ident);
   }

   LOG_DEBUG(PNET_MAIN_LOG, "APP: ret %d expected module call-back 0x%08x\n", ret, (unsigned)module_ident);
   ...
   uint32_t                submodule_ident)
{
   ...
   if (ix < NELEMENTS(submodule_cfg))
   {
      ...
      {
         LOG_DEBUG(PNET_MAIN_LOG, "APP: Pull sub-module failed\n");
      }
   ...
   LOG_DEBUG(PNET_MAIN_LOG, "APP: ret %d expected submodule call-back module ident 0x%08x, submodule ident 0x%08x\n", ret, (unsigned)module_ident, (unsigned)submodule_ident);
...
static int app_new_data_status_ind(
   ...
   uint8_t                 changes)
{
   LOG_DEBUG(PNET_MAIN_LOG, "APP: New data status 0x%02x\n", changes);
...
static int app_alarm_ind(
   ...
   uint8_t                 *p_data)
{
   LOG_DEBUG(PNET_MAIN_LOG, "APP: Alarm indicated\n");
...
{
   LOG_DEBUG(PNET_MAIN_LOG, "APP: Alarm confirmed: %u, %u, %u, %u\n",
...
static int app_alarm_ack_cnf(
   ...
   int                     res)
{
   LOG_DEBUG(PNET_MAIN_LOG, "APP: Alarm ACK confirmed: %d\n", res);
      ...
      char                 *argv[])
{
   ...
   if (main_arep != UINT32_MAX)
   {
      ...
      if (ret != 0)
      {
         LOG_ERROR(PNET_MAIN_LOG, "APP: Alarm ACK error %d\n", ret);
      }
      else
      {
         LOG_INFO(PNET_MAIN_LOG, "APP: Alarm ACK sent\n");
      }
   }
   else
   {
      LOG_INFO(PNET_MAIN_LOG, "APP: No AREP\n");
   }
#endif
   return 0;
}
      ...
      char                 *argv[])
{
   ...
   {
      LOG_DEBUG(PNET_MAIN_LOG, "APP: Call-backs connected\n");
      for (ix = 0; ix < NELEMENTS(submodule_cfg); ix++)
      {
         ...
         {
            LOG_ERROR(PNET_MAIN_LOG, "APP: Could not plug %u, %u, %u\n",
   ...
   {
      LOG_DEBUG(PNET_MAIN_LOG, "APP: Could not connect application call-backs\n");
   }
   return 0;
}
...
int main(void)
{
   ...
   LOG_INFO(PNET_MAIN_LOG, "APP: Hello world\n");
   ...
   for (;;)
   {
      ...
      if (flags & EVENT_READY_FOR_DATA)
      {
         ...
         LOG_DEBUG(PNET_MAIN_LOG, "APP: Return from apprdy: %d\n", ret);
         /*
      ...
      else if (flags & EVENT_TIMER)
      {
         ...
         if ((main_arep != UINT32_MAX) && (tick_ctr > 10))
         {
            ...
            //LOG_DEBUG(PNET_MAIN_LOG, "APP: Return from send  : %d\n", ret);
         }
#endif
#if 1
         /* Set new output when button 1 is pressed/released */
         pressed = os_get_button(0);

         if (main_arep != UINT32_MAX)
         {
            if ((pressed == true) && (old_1_pressed == false))
            {
               LOG_DEBUG(PNET_MAIN_LOG, "APP: New input\n");
               data[0] = data_ctr++;
               ret = pnet_input_set_data_and_iops(0, 1, 1, data, sizeof(data), PNET_IOXS_GOOD);
            }
         }
         ...
         pressed = os_get_button(1);

         if (main_arep != UINT32_MAX)
         {
            if ((pressed == true) && (old_2_pressed == false) && (alarm_allowed == true))
            {
               LOG_DEBUG(PNET_MAIN_LOG, "APP: Send ALARM\n");
               alarm[0]++;
               alarm_allowed = true;
      ...
      else if (flags & EVENT_ABORT)
      {
         ...
         LOG_DEBUG(PNET_MAIN_LOG, "APP: abort\n");
      }
   }
   os_timer_destroy(main_timer);
   os_event_destroy(main_events);

   LOG_INFO(PNET_MAIN_LOG, "APP: End of the world\n");
   return 0;
}
